...
public class KeyLoadUtil {
    ...
    public static String loadKeys(String privateKeyPath, String aesKeyPath) throws Exception {
        ...
        PrivateKey privateKey = KeyFactory.getInstance(RSA_ALGORITHM).generatePrivate(new PKCS8EncodedKeySpec(privateKeyData));

        return Base64.getEncoder().encodeToString(privateKey.getEncoded());
    }

    private static byte[] loadKey(String filePath) throws IOException {
        return Base64.getDecoder().decode(Files.readAllBytes(new File(filePath).toPath()));
    }

    private static byte[] decryptKey(byte[] encryptedKeyData, SecretKey secretKey) throws Exception {
